...
from builders import build  # noqa: E402
...
class TestListGeneratedInTarget(BaseTestCase):
    ...
    def test_encuentra_solo_con_marcador(self):
        ...
        write(self.target_dir, "manual.py", component_body("SIMPLE_MANUAL"))
        found = build.list_generated_in_target(self.target_dir)
        self.assertEqual([p.name for p in found], ["simple_tee.py"])
        self.assertEqual(found[0], gen)

    def test_no_incluye_protegidos(self):
        write(self.target_dir, "build.py", build.GENERATED_MARKER.format(src="x") + "\npass\n")
        write(self.target_dir, "__init__.py", "# AUTOGENERATED by build.py -- do not edit\n")
        self.assertEqual(build.list_generated_in_target(self.target_dir), [])
...
class TestOutputPath(BaseTestCase):
    def test_nombre_con_familia(self):
        ...
        self.assertEqual(build.output_path(self.target_dir, src, self.root).name, "tees.simple_tee.py")
        src2 = self.add_component("primitives/hollow_cylinder.py", "HOLLOW_CYLINDER")
        self.assertEqual(build.output_path(self.target_dir, src2, self.root).name, "primitives.hollow_cylinder.py")
...
class TestFlatten(BaseTestCase):
    def test_genera_outputs_con_marcador(self):
        ...
        build.flatten(self.root, self.target_dir)
        for src in (src1, src2):
            out = build.output_path(self.target_dir, src, self.root)
    ...
    def test_no_toca_archivos_manuales_sin_marcador(self):
        manual = write(self.target_dir, "manual.py", component_body("SIMPLE_MANUAL"))
    ...
    def test_actualiza_gitignore(self):
        self.add_component("tees/simple_tee.py", "SIMPLE_TEE")
        build.flatten(self.root, self.root)
    ...
    def test_devuelve_componentes(self):
        self.add_component("tees/simple_tee.py", "SIMPLE_TEE")
        result = build.flatten(self.root, self.target_dir)
...
class TestCollectErrors(BaseTestCase):
    def test_ok_cuando_sincronizado(self):
        self.add_component("tees/simple_tee.py", "SIMPLE_TEE")
        build.flatten(self.root, self.target_dir)
        self.assertEqual(build.collect_errors(self.root, self.target_dir), [])

    def test_error_si_falta_output(self):
        self.add_component("tees/simple_tee.py", "SIMPLE_TEE")
        errors = build.collect_errors(self.root, self.target_dir)
    ...
    def test_error_si_output_obsoleto(self):
        self.add_component("tees/simple_tee.py", "SIMPLE_TEE")
        build.flatten(self.root, self.target_dir)
        write(self.target_dir, "ghost.py",
    ...
    def test_error_doble_registro(self):
        ...
        with self.assertRaises(build.BuildError):
            build.collect_errors(self.root, self.target_dir)
